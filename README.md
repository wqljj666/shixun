...
- 用户输入症状、用药问题或健康咨询内容
- 支持同一 conversationId 下的连续多轮对话
- 后端会携带最近多轮上下文生成 AI 药师回复
- 支持配置大模型 API
- 未配置 API Key 时使用本地模拟回复，保证项目可运行
- 高风险关键词识别，且识别时会结合历史上下文
- 高风险问题提示转人工药师
- 保存咨询会话和多轮消息记录
- 查看历史咨询会话并加载完整聊天记录
...
9. 继续输入“会有什么风险？”，检查 AI 是否结合前一句上下文进行回复。
10. 输入高风险补充信息，例如“我有青霉素过敏史。”，检查是否出现高风险提示和转人工药师按钮。
11. 刷新页面后从最近咨询或咨询记录中打开历史会话，检查完整聊天记录是否保存。
